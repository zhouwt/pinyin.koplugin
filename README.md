...
> ⚠️ **本插件完全免费、开源（GPL-3.0）。请勿向任何第三方付费购买；直接来本仓库免费下载即可。**
...
| 拼音字号                 | 8–28,拼音大小(**建议 ≥ 26**,太小可能导致拼音显示不全/挤压;详见下「局限 / 已知问题」)                                             |
| 标注位置                 | **固定上方(不可更改)**                                                                                    |
...
```
[Pinyin] page=3 level=5 show_all=true thr=0 | boxes=142 words=138 chars=410 with_data=408 shown=408 filtered=0 no_data=2 no_word=4
```
...
* `no_word` — 盒子取不到汉字的情况(若偏高,说明 crengine 没返回完整坐标,是拼音缺失的主因)

开启后翻几页,把 `crash.log` 里这些行发回即可定位问题(是"取不到字"还是"数据缺失"还是"等级逻辑")。
...
1. `doc:getPageXPointer(page)` 取当前页首 XPointer,`page+1` 取页尾。
2. `doc:getScreenBoxesFromPositions(pos0, pos1, true)` 取得当前页每个可见片段
   (对中文通常每字一盒)的屏幕坐标盒子 `{x,y,w,h}`。
3. 对每个盒子用 `doc:getWordFromPosition(center, true)` 取得对应汉字。
4. 查 `pinyin_data.lua`:`data[字] = "拼音|排名"`。按等级阈值决定要不要标注。
5. 用 `TextWidget` 把拼音 `paintTo` 到屏幕缓冲 `Screen.bb` 上。
6. 关键:通过 `UIManager:setDirty(self.ui.dialog, function() ... end)` 的**函数形式**
   绘制——KOReader 会先重绘页面、再调用本函数画拼音、最后刷新该区域,
   因此拼音能正确叠加在文字之上并随刷新保留。
...
* **拼音字号过小会影响展示**:拼音是按固定像素字号叠加在汉字上方的。若「拼音字号」设得太小(如 8–20),拼音很可能**显示不全、被压扁或与汉字重叠**,看起来像"没注音"。**建议把字号调整到 26 以上**;若仍偏小,可继续调大或增大页边距来获得足够空间。
